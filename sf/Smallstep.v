...
Proof.
  ...
  apply H.
  exists (C 0). apply ST_PlusConstConst with (n1:=0) (n2:=0).
Qed.
(** [] *)
End Temp1.
...
Proof.
  ...
  unfold normal_form. unfold not. intros.
  apply H.
  exists (P (C 0) (C 0)). apply ST_Funny with (n:=0).
Qed.
...
Proof.
  ...
  unfold normal_form. unfold not. intros. inversion H. inversion H0. inversion H4.
Qed.
(** [] *)
...
Proof.
  ...
  left. constructor.
  right.
  inversion IHt1. inversion IHt2. inversion H.
...
Proof.
  ...
  subst. apply IHstep in H5. rewrite H5. reflexivity.
Qed.
(** [] *)
...
Lemma multistep_congr_2 : forall t1 t2 t2',
     ...
     P t1 t2 ==>* P t1 t2'.
Proof.
  intros. induction H. induction H0. apply multi_refl.
  apply multi_step with (P (C n) y).
  apply ST_Plus2. constructor. apply H. apply IHmulti.
Qed.
(** [] *)
...
Proof.
  ...
  apply multistep_congr_2. constructor. apply IHeval2.
  apply multi_R.
  apply ST_PlusConstConst.
Qed.
...
Proof.
  ...
  induction Hs. intros. inversion H. subst. apply E_Plus. apply E_Const. apply E_Const.
  intros. inversion H. subst. apply E_Plus. apply IHHs in H2. apply H2. apply H4.
  intros. inversion H0. subst. apply E_Plus. apply H3. apply IHHs in H5. apply H5.
Qed.
(** [] *)
...
Proof.
  ...
  intros. inversion H. induction H0. apply nf_is_value in H1. subst. inversion H1. exists n. split. reflexivity. apply E_Const.
  subst.  apply nf_is_value in H1. inversion H1. exists n. split. reflexivity. apply step__eval with (y). assumption. assert (y ==>* z /\ normal_form step z). split. apply H2. apply H.  apply IHmulti in H4. inversion H4. rewrite <- H3 in H5. inversion H5. inversion H6. apply H7.  apply H.
Qed.
(** [] *)
...
Proof.
  ...
  apply IHt1 in H2. apply IHt2 in H4. omega.
Qed.
(** [] *)
...
Proof.
  ...
  eapply multi_step. apply CS_Par2. rewrite H1. apply CS_IfStep. apply BS_Eq. simpl.
  eapply multi_step. apply CS_Par2. apply CS_IfTrue.
