...
Proof.
  induction l.
  - reflexivity.
  - simpl. assert(H: rev(snoc (rev l) n) = n :: rev (rev l)).
...
Proof.
  induction l.
  - reflexivity.
  - simpl. assert(H: beq_nat n n = true).
...
Proof.
  intros.
  assert (H0: forall l: natlist, l = rev (rev l)).
    - intros. rewrite rev_involutive. reflexivity.
    - intros. symmetry. rewrite H0. rewrite H. rewrite rev_involutive. reflexivity.
Qed.
